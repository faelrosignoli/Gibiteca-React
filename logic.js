...
function renderGenreGrid(){ const sel=editorDraft.genres||(editorDraft.genres=[]);
  $('#genreGrid').innerHTML = GENRES.map(g=>`<button type="button" class="gchip ${sel.includes(g)?'on':''}" data-g="${esc(g)}">${esc(g)}</button>`).join('');
  $$('#genreGrid .gchip').forEach(b=>b.onclick=()=>{ const g=b.dataset.g; const i=editorDraft.genres.indexOf(g);
    if(i>=0) editorDraft.genres.splice(i,1); else editorDraft.genres.push(g); b.classList.toggle('on'); }); }
function openGenrePopup(){ renderGenreGrid(); $('#genreOverlay').classList.add('on'); }
